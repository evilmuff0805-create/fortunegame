...
- **시뮬에서 '귀'를 '뿔'로 착각함 — 기기 확인이 진짜 검증.** 사슴 베레모 0.14/폭0.52가 시뮬상 "뿔 노출"로 보였으나
  실기기에선 모자가 커서 뿔을 덮고 **옆에 보인 건 귀**였음. → 모자 **너비도 동물별로**(사슴 0.34) 줄이고 overlap 0.18로.
  교훈: 부착물 충돌은 width+overlap 둘 다 변수. 작은 차이는 실기기 스크린샷으로 최종 확인(시뮬 과신 금지).
- **플레이스홀더 합성은 홈에서 숨기고 꾸미기 미리보기에서만.** 아트 없는 손소품/배경 칩이 홈에서 "떠다니는 박스"로
  보임 → `showPlaceholderItems`(홈 false/꾸미기 true). 실제 아트 있는 아이템(베레모)은 홈에도 합성. 깔끔함+검증 양립.
