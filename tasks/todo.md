...
## Slice 3 — 데일리 루프 (코어 가설) ✅ (2026-06-17)

> 동물 비주얼·연출은 gito만 실제 아트, 나머지 9종+봉투/아이템은 플레이스홀더 → **흐름만 검증, 감동 평가는 아트 후**

- [x] 홈 화면: 내 동물(gito는 deliver 포즈) + 오늘 봉투(미개봉/개봉) + 스트릭
- [x] 찢기 인터랙션: **드래그 진행도=찢김 실시간**(GestureDetector+CustomPainter), 단계별 햅틱(약·중·강·완료), 탭=바로 열기(스킵)
- [x] 운세 카드 화면: 날씨 등급 + 4카테고리 점수 막대 + 메시지
- [x] 보상 지급: 등급→아이템 고정 매핑(D3), **open-pack 서버 권위 inventory INSERT**(멱등). 수령 연출 심플
- [x] 무지개/쾌청 사전 누설: 미개봉 시 누설 티어만 노출(①), 봉투 틈 무지갯빛/금빛 + 홈 카드 힌트
- [x] 스트릭 로직: `_shared/streak.ts` 순수함수 + **KST 자정 경계 테스트(23:59→00:01)** 등 8건 통과. D5: 끊겨도 협박 없음
- [x] 공유 카드 ②: "오늘의 행운"(맑음 이상 노출) + shared_lucky + share_events
- [x] 아침 로컬 푸시: flutter_local_notifications+timezone(KST), 시간 설정 가능, 협박 문구 없음
- [x] 분석: pack_opened·shared_lucky 발화
- [x] Verify: 백엔드 E2E(개봉→보상·스트릭·opened_at, 멱등, 누설 숨김, 파밍 403) / analyze 0·test 7·APK
      → **이틀 연속 실기기 개봉·날짜전환·푸시 전달은 실기기 이연(컨테이너 불가)**

## Slice 4 — 꾸미기 + 앨범 ✅ (2026-06-17)

- [x] 꾸미기: 슬롯 3개(모자/손소품/배경), **골격가이드 §3 정규화 앵커 합성**(하드코딩 픽셀 0). 미소유 미리보기 OK·장착은 소유만(🔒)
- [x] equipped 저장(클라 CRUD) → 홈 DressedAnimal에 반영
- [x] 앨범: 월별 7열 그리드, 개봉일=등급 카드(탭 상세)/미개봉·무기록 과거=회색 빈슬롯/미래=빈칸. opened_at 기준. 협박 없는 카피(D5)
- [x] 곁다리: 동물 미세 idle 애니메이션(코드 only 호흡, 강도 `IdleAnim` 상수 — 폰에서 조정). 10종 공용
- [x] 버그수정: 보상 "을(를)" → 받침 판정 조사 함수(`core/korean.dart`)
- [x] Verify: analyze 0·test 9(조사·앵커 합성 포함)·APK
      → **장착 재실행 유지·하루 건너뛴 빈슬롯·베레모 장착(이메일 연결 후 주입)은 실기기 도그푸딩**
...
**무엇이 바뀌었나:**
- 서버(결정 1): `open-pack`(개봉=opened_at+보상 inventory+스트릭, 원자·멱등), `today-fortune` 개편(미개봉=누설 티어만 ①),
  `_shared/fortune.ts`(등급 단일 경로 ③)·`_shared/streak.ts`·`_shared/reward.ts`
- 마이그레이션: items 5종 시드 + inventory/streaks 쓰기 RLS 잠금(서버 전용)
- 클라: 데일리 루프 홈 재구성, 봉투 찢기(드래그=찢김+햅틱+스킵+누설), 운세 카드(등급·점수·메시지·보상),
  공유 카드 ②, 로컬 푸시 서비스, 아이템/등급 모델

**어떻게 검증했나:**
- deno: streak 8건(자정/월말/연말 경계, 리셋, 멱등) / 함수 4개 타입체크
- REST E2E: today-fortune 미개봉=leak만(cloudy 숨김) → open-pack=cloudy/charm_comfort01/스트릭1 →
  재호출 멱등(justOpened=false, 보상·스트릭 불변) → 클라 직접 inventory INSERT **403**(파밍 차단) → opened_at 기록(D4)
- flutter analyze 0 / test 7건(찢기 스킵·등급 공유성 포함) / APK 빌드

**다음 슬라이스 전 사용자 액션:**
- 실기기: 이틀 연속 개봉·KST 날짜전환·푸시 전달 수동 확인 (컨테이너 불가)
- (선택) 나머지 9종 + 봉투/아이템 아트 양산 → 감동 평가
